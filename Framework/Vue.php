...
class Vue {
    ...
    public function generer($donnees, $requete) {
        ...
        if ($requete != null && $requete->getSession()->existeAttribut("idUtilisateur")) {
            $utilisateur['id'] = $requete->getSession()->getAttribut("idUtilisateur");
            $utilisateur['login'] = $requete->getSession()->getAttribut("login");
            $utilisateur['nom'] = $requete->getSession()->getAttribut("nomUtilisateur");
        } else {
            $utilisateur = '';
        }
        // Génération de la partie spécifique de la vue
        $contenu = $this->genererFichier($this->fichier, $donnees, $utilisateur);
        // Génération du gabarit commun utilisant la partie spécifique
        $vue = $this->genererFichier('Vue/gabarit.php', array('titre' => $this->titre, 'contenu' => $contenu,
            'racineWeb' => $racineWeb), $utilisateur);
     ...
     */
    private function genererFichier($fichier, $donnees, $utilisateur) {
